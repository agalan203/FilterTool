...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
        ...
        if reply == QMessageBox.Yes:
            ...
            if filename:
                #try:
                    ...
                    self.spin_wp.setValue(self.designconfig.wp / (2*pi))
                    self.spin_wa.setValue(self.designconfig.wa / (2*pi))
                    self.spin_wp_2.setValue(self.designconfig.wp2 / (2*pi))
                    self.spin_wa_2.setValue(self.designconfig.wa2 / (2*pi))
    ...
    def plotAll(self, designconfig):
        ...
        if len(warning_msg) > 0:
            ...
            try:
                ...
                freq1 = Gain[0] / (2* pi)
                self.getPlotAxes('Atenuación').semilogx(freq, Attenuation[1], 'k')
                self.getPlotAxes('Fase').semilogx(freq1, Gain[2], 'k')
                self.getPlotAxes('Respuesta Total').semilogx(freq1, Gain[1], 'k')
    ...
    def plotPolesAndZeros(self, z, p):
        ...
        for i, pole in enumerate(p):
            ...
            poles_labels[xy] = 'Polo n°' + str(i + 1) if xy not in poles_labels else poles_labels[xy] + ', ' + str(i + 1)
